              ...
              <div key={product.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{product.itemPrefix}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-white">{product.currency} {Number(product.price).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Draw Date</p>
                    <p className="text-gray-900 dark:text-white">{new Date(product.drawDate).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Sold/Total</p>
                    <p className="text-gray-900 dark:text-white">{product.soldItems} / {product.totalItems}</p>
                  </div>
                </div>
                <Link href={`/admin/products/${product.id}`} className="block w-full px-4 py-2 bg-blue-600 text-white text-center rounded-lg hover:bg-blue-700 transition text-sm">
                  Edit Product
                </Link>
              </div>
            ))}
